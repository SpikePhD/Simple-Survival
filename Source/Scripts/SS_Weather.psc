...
Function EvaluateWeather(String source = "Tick")
  ...
  if coldOn
    int eventHandle = ModEvent.Create("SS_SetCold")
    if eventHandle
      ModEvent.PushInt(eventHandle, healthPenaltyPct)
      ModEvent.PushInt(eventHandle, staminaPenaltyPct)
      ModEvent.PushInt(eventHandle, magickaPenaltyPct)
      ModEvent.PushInt(eventHandle, speedPenaltyPct)
      ModEvent.PushInt(eventHandle, preparednessTier)
    ...
    int clearHandle = ModEvent.Create("SS_ClearCold")
    if clearHandle
      ModEvent.Send(clearHandle)
      if bDebugEnabled
